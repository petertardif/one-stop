...
| Column | Notes |
|--------|-------|
| ☑ | Row select checkbox (leftmost), enabled on **every** row incl. Plaid. Header checkbox = select-all, which adds/removes the **visible page only**. The selection **persists across pagination and filter changes** (cleared by the Checkboxes Total card's Clear, or as bulk actions consume rows). Drives the toolbar Delete/Duplicate/Mark as… actions (manual rows only) and the Checkboxes Total card. |
...
**Toolbar actions:** `Clear Filters` (conditional), `Mark as…`, `Duplicate`, `Delete`, `Add` (right group). **`Add`** is a filled-primary **dropdown** (chevron; same open/outside-click/Escape behavior as `Mark as…`) with two icon options — **New Transaction** (opens the add-transaction modal) and **Amount to 1k Weekly** (opens the weekly top-up modal). Every other button (including `Delete`) is an accent-outline secondary; every button carries an icon + label. **`Sync` is hidden** (gated behind `SHOW_SYNC` in `MonthlyLedger.tsx`) until Plaid + an account-setup flow exist; it will return to the left of the toolbar then.
...
**Checkboxes Total card:** ticking any row(s) reveals a third card in the row above the filters, titled **Checkboxes Total**, summing the `amount` of every checked row — **green** when the sum is positive, **red** when negative, **black** at exactly `$0.00` (summed in whole cents so a netting-out selection reads as a true zero). Formatted `$1,234.56` / `-$1,234.56`. Its secondary line reads `N selected — Clear`, where **Clear** wipes the entire selection including rows on other pages. Because selection survives paging and filters, amounts are cached client-side at check time (an off-page row is not in the fetched page) and re-synced whenever a selected row appears in freshly fetched data, so editing a row updates the total.

**Clear Filters:** whenever the Category or Posted column filter is narrowed, a **Clear Filters** button appears in the toolbar's bulk-action group, **directly left of `Mark as…`**, resetting both to their defaults. Period is not included (it always holds a value, so the button would never hide). Present in the mobile Filters modal too.

**Bulk actions:** Row checkboxes drive several toolbar buttons, enabled once ≥1 **manual** row is selected. Since imported rows are now selectable for totaling, each action operates on the **manual subset** of the selection and leaves the rest checked:
- **Delete** — opens a confirm modal ("You are about to delete X transactions. Do you want to proceed?"), which additionally reports how many imported rows can't be deleted and will be skipped; on confirm, bulk-deletes the selected manual rows and re-shifts the running `balance` of later rows. Plaid rows stay protected (also enforced server-side).
...
- **Paid With** — which household account the debt is paid from, mirroring the ledger's **Budget** column: **N/A** (null) · **Bank** · **Chase CC** · **BoA CC**, sharing its options and read-view icons (`components/BudgetAccount.tsx` — landmark for Bank, Chase-blue card, BoA-red card, each with a "Paid with …" tooltip; N/A renders nothing). Editable inline (select) and offered in the **New Debt** modal, defaulting to **N/A**. Sortable by label; not part of the search box. Shown on the **Short Term** and **Paid Off** tabs only — **Long Term** debts don't carry one, so the field disappears from the modal when Term is Long and the value is cleared server-side whenever a debt's term is `long`. A long-term debt therefore always reads N/A once marked paid.
...
- `POST /api/debts` — create account `{ name, category, term, paid_with?, balance?, as_of? }` (admin); appends to the tail. `paid_with` is forced to null when `term = 'long'` (shared `paidWithFor` in `lib/debts.ts`). When `balance` is supplied it writes the debt's first `debt_snapshots` row (dated `as_of`, default `CURRENT_DATE`) in the same transaction
- `PUT /api/debts/[id]` — edit name / category / term / paid_with (admin); same `paid_with` null-on-long rule as create
...
debt_accounts       id, user_id (FK), name, category (text, nullable — free-form debt taxonomy), term (short|long), paid_with (nullable — bank|chase_cc|boa_cc; null = N/A; always null when term='long', enforced by the API), sort_order (int — persisted manual drag order), paid_at (nullable — set = Paid Off tab), created_at, updated_at
